...
private:
	...
	template<uint8_t INTERRUPT_PIN>
	void sleep(void (*wakeCallback)(void), uint8_t interruptType, uint32_t interruptDuration, uint32_t chargeDuration) {
		// flash LED
		pinMode(13, OUTPUT);
		for (int i = 0; i < 3; i++) {
			digitalWrite(13, LOW);
		...
		if (chargeDuration > 0) {
			...
			while (chargeDuration >= millis() - chargeTime) {
				...
				if (this->confirmInterrupt(INTERRUPT_PIN,
							!chargeInterrupt, interruptDuration)) {
					this->setup();
					if (wakeCallback) {
						wakeCallback();
					}
		...
		if (this->confirmInterrupt(INTERRUPT_PIN, desiredPinState, interruptDuration)) {
			...
			this->sleep<INTERRUPT_PIN>(wakeCallback, interruptType, interruptDuration, 0);
		}
	}
public:
	...
	void update(bool (*sleepCallback)(void), void (*wakeCallback)(void), void (*loopCallback)(void)) {
		...
		if (sleepCallback()) {
			...
			this->sleep<INTERRUPT_PIN>(wakeCallback, INTERRUPT_TYPE, INTERRUPT_DURATION, CHARGE_DURATION);
		}
	}
};
